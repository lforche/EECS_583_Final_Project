 ...
 *     for details.
 ************************************************************/
/* Removed ALTIVEC stuff as suggested by John Henning and Cloyce 
   Kaivalya Tue Dec  2 16:33:43 CST 2003
*/

/* fast_algorithms.c
 * SRE, Sun Nov 10 08:54:48 2002 [AA 3080, Denver to StL]
 * CVS $Id: fast_algorithms.c,v 1.1 2007/01/25 17:20:07 mchu Exp $
 * 
...
P7Viterbi(char *dsq, int L, struct plan7_s *hmm, struct dpmatrix_s *mx, struct p7trace_s **ret_tr)
{
  ...
  for (i = 1; i <= L; i++) {
    ...
    for (k = 1; k <= M; k++) {
        ...
        int my_mc = mpp[k-1] + tpmm[k-1];
        if ((sc = ip[k-1]  + tpim[k-1]) > my_mc)  my_mc = sc;
        if ((sc = dpp[k-1] + tpdm[k-1]) > my_mc)  my_mc = sc;
        if ((sc = xmb  + bp[k])         > my_mc)  my_mc = sc; 
        my_mc += ms[k];
        if (my_mc < -INFTY) my_mc = -INFTY;  
        mc[k] = my_mc;

        int my_dc = dc[k-1] + tpdd[k-1];
        if ((sc = mc[k-1] + tpmd[k-1]) > my_dc) my_dc = sc;
        if (my_dc < -INFTY) my_dc = -INFTY;  
        dc[k] = my_dc;
        __asm__ volatile ("xchg %r13, %r13");
#else
        __asm__ volatile ("xchg %r13, %r13");
...
#endif /* do not allow ALTIVEC stuff for SPEC port Kaivalya 12/02/03 */
